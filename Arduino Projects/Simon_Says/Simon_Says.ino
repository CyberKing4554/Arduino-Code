...
int button[] = {9,2,12,6};     //red is button[0], yellow is button[1], green is button[2], blue is button[3]
int led[] = {10,3,13,5};        //red is led[0], yellow is led[1], green is led[2], blue is led[3]
int tones[] = {262, 330, 392, 494};   //tones to play with each button (c, e, g, b)

int roundsToWin =7;         //number of rounds the player has to play before they win the game (the array can only hold up to 16 rounds)
...
for(int i=0; i <= roundCounter; i++){   //for each button to be pressed in the sequence
  ...
  while(true){  //loop until the player presses a button or the time limit is up (the time limit check is in an if statement)
    ...
    if(millis() - startTime > timeLimit){
      Serial.println(roundCounter);//if the time limit is up
